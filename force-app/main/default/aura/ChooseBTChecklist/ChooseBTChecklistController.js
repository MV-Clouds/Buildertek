({
  ...
  checkTheObject: function (component, event, helper) {
    if (
      component.get("v.SelectedChecklistObject") != undefined &&
      component.get("v.SelectedChecklistObject") != "" &&
      component.get("v.SelectedChecklistObject") != "--None--"
    ) {
  ...
  checkObjName: function (component, event, helper) {
    if (
      component.get("v.ChecklistName") != "" &&
      component.get("v.ChecklistName") != null &&
      component.get("v.ChecklistName") != undefined
    ) {
  ...
  ChageType: function (component, event, helper) {
    ...
    if (
      newQua.buildertek__Question_Type__c == "Single Select Answer" ||
      newQua.buildertek__Question_Type__c == "Multi Select Answer"
    ) {
  ...
  SaveQuestionsModel: function (component, event, helper) {
    ...
    if (errormsg == false) {
      if (
        component.get("v.groupName") != null &&
        component.get("v.groupName") != "" &&
        component.get("v.groupName") != undefined
      ) {
      ...
      action.setCallback(this, function (a) {
        if (a.getState() === "SUCCESS") {
          ...
          if (result == "Success") {
            ...
            component.doinits();
            if (
              component.get("v.configureId") == undefined ||
              component.get("v.configureId") == ""
            ) {
              var toastEvent = $A.get("e.force:showToast");
              toastEvent.setParams({
                title: "Success!",
  ...
  callUpdateRecord: function (component, event, helper) {
    ...
    action.setCallback(this, function (a) {
      if (a.getState() === "SUCCESS") {
        ...
        component.set(
          "v.SelectedChecklistObject",
          result.buildertek__Object_Name__c
        );
        if (
          result.buildertek__Checklist_Questions__r != undefined &&
          result.buildertek__Checklist_Questions__r != "" &&
          result.buildertek__Checklist_Questions__r != null
        ) {
          for (
            var i = 0;
            i < result.buildertek__Checklist_Questions__r.length;
            i++
          ) {
            //alert(result.buildertek__Checklist_Questions__r[i].buildertek__Questions__r.Id);
            if (
              result.buildertek__Checklist_Questions__r[i]
                .buildertek__Questions__r != undefined
            ) {
              var data =
                result.buildertek__Checklist_Questions__r[i]
                  .buildertek__Questions__r.Id;
              document
                .getElementById("Selected")
                .appendChild(document.getElementById(data));
              helper.refreshList(component, event);
  ...
  callEditQuestion: function (component, event, helper) {
    ...
    action.setCallback(this, function (a) {
      if (a.getState() === "SUCCESS") {
        ...
        if (
          result.buildertek__Question_Type__c == "Single Select Answer" ||
          result.buildertek__Question_Type__c == "Multi Select Answer"
        ) {
          component.set("v.isMulti", true);
        } else {
          component.set("v.isMulti", false);
  ...
  AddGroup: function (component, event, helper) {
    console.log("calling add group method");
    component.set("v.showGroup", false);
